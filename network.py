...
class Net(object):
    ...
    @staticmethod
    def softmax(scores):
      """ Get softmax values of 2D numpy array scores"""
      e_scores = np.exp(scores)
      return e_scores/e_scores.sum(axis=1)[:,None]
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #############################################################################
        # your code
        # Variables along network: input - fully connected layer - ReLU - fully connected layer - softmax
        #                             X  -          Y1        -   Y1_bar  -       scores
        Y1 = np.dot(X,W1) + b1
        Y1_bar = np.maximum(np.zeros((N, W1.shape[1])),Y1)
        scores = np.dot(Y1_bar,W2) + b2
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        #############################################################################
        # your code
        softmax = Net.softmax(scores)
        data_loss = np.sum(np.negative(np.log(np.choose(y, softmax.T)))) / N
        ...
        #############################################################################
        # your code
        y_vects = np.zeros((N,C))
        y_vects[np.arange(N), y] = 1   # one-hot encoded vector of y
        dsoftmax_dscores = softmax - y_vects
        grads['W2'] = np.dot(Y1_bar.T,dsoftmax_dscores) / N + (2*reg*W2)
        grads['b2'] = dsoftmax_dscores.sum(axis=0)/N

        dscores_dY1bar = W2
        dsoftmax_dY1bar = np.dot(dsoftmax_dscores, dscores_dY1bar.T)
        dreLU = np.zeros(Y1_bar.shape)
        dreLU[Y1_bar>0] = 1
        grads['W1'] = np.dot(X.T, dreLU * dsoftmax_dY1bar) / N + (2*reg*W1)
        grads['b1'] = (dsoftmax_dY1bar * dreLU).sum(axis=0) / N
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            if batch_size < num_train:
              idx = random.sample(range(num_train), batch_size)
              X_batch = X[idx, :]
              y_batch = y[idx]
            else:
              X_batch = X
    ...
    def predict(self, X):
        """
        USING THE TRAINED WEIGHTS OF THE NETWORK FOR PREDICTING THE LABELS: 
